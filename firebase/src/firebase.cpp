...
static dmScript::LuaCallbackInfo* g_InstallationIdCallback;
...
static int Firebase_GetInstallationId(lua_State* L) {
	DM_LUA_STACK_CHECK(L, 0);

	g_InstallationIdCallback = dmScript::CreateCallback(L, 1);
	...
	installations_object->GetId()
		...
		{
			dmLogWarning("Firebase installation id callback is not valid");
			return;
		}

		if (dmScript::SetupCallback(g_InstallationIdCallback))
		{
			lua_State* L = dmScript::GetCallbackLuaContext(g_InstallationIdCallback);
			...
			dmScript::TeardownCallback(g_InstallationIdCallback);
		}
...
static void LuaInit(lua_State* L) {
	...
	lua_pushtablestringfunction(L, "get_installation_id", Firebase_GetInstallationId);
